...
## Economy and divergence (the 2026-09-08 sessions: eight full covers built before anyone chose, all from one library)
13. **Cheapest choice first; build only the pick.** Never build what nobody chose. Put the choice in front of the human at the lowest fidelity that lets them decide: a slate of 3 to 4 CHEAP candidates (a library demo screenshot, a reference capture, at most one static render each) on ONE contact sheet, then build ONLY the picked direction. Variants of the pick only if the human asks. Fail fast: when a direction dies (no asset, a broken component, a source that will not load), drop it in one line and move on; never fix what the choice does not need. Build nothing the brief and the direction do not name. ENFORCED: `run_state.py candidate --add` refuses a `built` candidate before a direction is recorded and refuses a fifth candidate in any slate (SLATE_MAX=4); `pick` refuses anything not registered as built; the Stop hook does not let a turn end "waiting on the human" until the slate they choose from is registered.
14. **Divergent slates, never derivatives.** A direction slate comes from different wells: at least two sources (libraries, mcp-image, a brand asset, type-only) and at least two archetypes (type-led, photo-led, motion-led, component-grid, generated-imagery, ...), and no two candidates may share source and archetype. Eight Aceternity effects are one direction shown eight times. ENFORCED: `run_state.py direction --candidate` refuses a slate that fails this. Ask the scout for a spread, not a ranking. If the human named the direction up front, record it with `direction --given '<their words>'` and skip the slate; that is the only bypass, and it is visible in the run file.
