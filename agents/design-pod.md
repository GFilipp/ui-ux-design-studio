...
- Render the live build headless, mobile-first: `engine/floor/render.mjs <url>` then `engine/floor/floor_check.py --require-assets` at 390 then 1440. Pass `--require-assets` or the `assets` gate is pure assertion. Set `contrast`, `orphans`, `layout`, `console`, `targets`, `type_size`, `measure` from the floor result; set `responsive` only after BOTH breakpoints pass.
